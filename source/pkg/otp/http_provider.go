...
func (p *HTTPProvider) Send(ctx context.Context, phone, message string) error {
	...
	if err != nil {
		extlog.Call("otp", "send_sms", http.MethodGet, requestURL, "", 0, "", durationMs, err.Error())
	...
	if err != nil {
		extlog.Call("otp", "send_sms", http.MethodGet, requestURL, "", resp.StatusCode, "", durationMs, err.Error())
		return fmt.Errorf("failed to read SMS gateway response: %w", err)
	}

	// PR #304: sorğu+cavab Loki-yə (parol URL-də avtomatik maskalanır)
	extlog.Call("otp", "send_sms", http.MethodGet, requestURL, "", resp.StatusCode, string(body), durationMs, "")
