...
--
-- IMPORTANT: We must drop ALL constraints and indexes that reference
-- this column before we can ALTER COLUMN. Migration 001 created the
-- column with `DEFAULT GETDATE()` which auto-generates a constraint
-- name (not DF_application_checks_checked_at as we assumed). We use
-- dynamic SQL to find and drop any such constraint.
-- ============================================================
...
-- (We loop through all indexes and drop those that include the column)
