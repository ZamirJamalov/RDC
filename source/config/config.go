...
type Config struct {
	...
	// Should ONLY be true in dev/test environments (set explicitly: MIGRATIONS_DROP_RECREATE=true).
	// PR #294: default false (fail-closed) — prod-da env verilməsə belə data güvəndədir.
	...
	MyGovWebURL      string // Web URL for SMS (mygov:// deeplink bridge page — hazırda linkmygov.netlify.app; öz web/mygov.html-imiz alpul.az aktivləşəndə host olunacaq)
	...
	// PR #404/#430: LW partner phones — aktivdir (approve-da 3 kontakt nömrəsi
	// LW-yə göndərilir). Kod səviyyəsində idarə olunur, env-dən oxunmur.
...
func Load() *Config {
	cfg := &Config{
		...
		MigrationsDropRecreate: getEnvBool("MIGRATIONS_DROP_RECREATE", false), // PR #294: default false — dev-də açıq şəkildə true yazılır
		...
		MyGovWebURL:            getEnv("MYGOV_WEB_URL", "https://linkmygov.netlify.app/"),
		...
		// (application create-dən ƏVVƏL; xəta olsa approval bloklanır — retryable).
		LwPartnerPhonesEnabled: true,
	...
	if cfg.MigrationsDropRecreate {
		slog.Warn("MIGRATIONS_DROP_RECREATE is true — tables will be dropped and recreated on startup. " +
			"Bu yalnız dev/test üçündür (PR #294-dən etibarən default false-dur, açıq şəkildə true qoyulub).")
	}
