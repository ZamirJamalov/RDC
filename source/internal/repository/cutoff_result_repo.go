...
func (r *CutoffResultRepo) Insert(ctx context.Context, cr *model.CutoffResult) error {
	_, err := r.db.ExecContext(ctx, `
		INSERT INTO cutoff_results
			(application_id, cutoff_code, cutoff_name, service_name, checked, passed, actual_value, threshold, details)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		cr.ApplicationID, cr.CutoffCode, cr.CutoffName, cr.ServiceName,
		cr.Checked, cr.Passed, cr.ActualValue, cr.Threshold, cr.Details,
	)
	if err != nil {
...
func (r *CutoffResultRepo) ListByApplication(ctx context.Context, appID int) ([]model.CutoffResult, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT id, application_id, cutoff_code, cutoff_name, service_name,
		       checked, passed, actual_value, threshold, details, created_at
		FROM cutoff_results
	...
	for rows.Next() {
		...
		var serviceName, actualValue, threshold, details sql.NullString
		if err := rows.Scan(
			&cr.ID, &cr.ApplicationID, &cr.CutoffCode, &cr.CutoffName, &serviceName,
			&cr.Checked, &cr.Passed, &actualValue, &threshold, &details, &cr.CreatedAt,
		); err != nil {
