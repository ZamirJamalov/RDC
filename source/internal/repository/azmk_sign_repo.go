...
//        - signed=true   → Disburse → MarkDisbursedIfPendingSignature
...
// MarkDisbursedIfPendingSignature atomically transitions pending_signature →
// disbursed. Returns false if the row was not in pending_signature status
// (already changed by a concurrent worker / manual action).
//
// Guarded update: iki worker eyni anda işə düşsə disburse bir dəfə sayılır.
func (r *ApplicationRepo) MarkDisbursedIfPendingSignature(ctx context.Context, id int) (bool, error) {
	res, err := r.db.ExecContext(ctx, `
		UPDATE loan_applications
