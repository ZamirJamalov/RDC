...
func (r *ApplicationRepo) GetApplicationByID(ctx context.Context, id int) (*model.LoanApplication, error) {
	...
	var contact1, contact2, contact3, contact1Rel, contact2Rel, contact3Rel, contact1Name, contact2Name, contact3Name, address, registrationAddress, customerPhone, customerSerial sql.NullString
	...
	err := r.db.QueryRowContext(ctx, `
                SELECT id, public_id, customer_pin, customer_full_name, amount, term_months, loan_purpose,
                       ...
                       official_income, contact1_phone, contact2_phone, contact3_phone, actual_address, registration_address,
	...
	// PR #124: kontakt yoxlanma statusu
	if contact1Verified.Valid {
		v := contact1Verified.Bool
		app.Contact1Verified = &v
	}
	if contact2Verified.Valid {
		v := contact2Verified.Bool
		app.Contact2Verified = &v
	}
	if contact3Verified.Valid {
		v := contact3Verified.Bool
		app.Contact3Verified = &v
	}
	app.ActualAddress = address.String
	// PR #245: qeydiyyat ünvanı (AZMK GetPersonalInfo)
	app.RegistrationAddress = registrationAddress.String
...
// PR #288: validity_days=99999 (=274 il) — daimi blok üçün unikal konvensiya;
// köhnə validity_days=0 rows da daimi qəbul olunur (backward compat).
...
func (r *ApplicationRepo) checkLastRejectionCutoff(ctx context.Context, customerPIN string) (int, string, int, error) {
	...
	// PR #288: permanent rejection. validity_days = 99999 — daimi blok (unikal
	// konvensiya). 0 da daimi kimi qəbul olunur (köhnə rule-lar üçün backward
	// compat — 0 heç vaxt "dərhal icazə" demək deyil).
	if validityDays == 0 || validityDays >= 99999 {
