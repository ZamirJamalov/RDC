...
// ExpireActiveByPhone marks all active OTP codes for the given phone as
// 'expired'. Called before inserting a new OTP code to avoid collision
// with the filtered unique index UQ_otp_codes_phone_active (which only
// allows one active code per phone at a time).
//
// This is correct behavior: if a customer requests a new OTP, any previous
// active (un-verified) code for that phone is no longer relevant and should
// be expired.
func (r *OTPRepo) ExpireActiveByPhone(ctx context.Context, phone string) error {
        _, err := r.db.ExecContext(ctx, `
                UPDATE otp_codes
                SET status = 'expired'
                WHERE phone = ? AND status = 'active'`,
                phone)
        if err != nil {
                return fmt.Errorf("failed to expire active OTP codes: %w", err)
        }
        return nil
}
