...
func TestBuildRangeSummary(t *testing.T) {
	tests := []struct {
		...
		{"empty", nil, 1, true, ""},
		{"single range phase 1", []repository.LevelRange{{MinAmount: 100, MaxAmount: 500, TermMonths: 3, Commission: 30.0, Phase: 1}}, 1, false, "100-500 AZN"},
		{"multiple ranges phase 2", []repository.LevelRange{{MinAmount: 100, MaxAmount: 300, TermMonths: 3, Commission: 29.0, Phase: 1}, {MinAmount: 301, MaxAmount: 500, TermMonths: 3, Commission: 28.0, Phase: 2}}, 2, false, "100-500 AZN"},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
