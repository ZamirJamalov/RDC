...
//
// The request includes the application ID, amount, credit level, and term.
// Card number is not currently collected by the application form — it's left
// empty here and will be populated when the form is extended (Phase 5 / T-5.3).
//
...
func (e *CreditEngine) notifyLwApproval(ctx context.Context, app *model.LoanApplication, creditLevel string, decision *decisionResult) error {
	req := &lw.ApproveLoanRequest{
		...
		CardNumber:    "", // TODO: T-5.3 — collect card number in application form
