...
// history) and the sequential checks (credit level + blacklist), returning the
// aggregated results.
//
...
// on LW or DB). The credit-level and blacklist checks run sequentially after
// them: credit-level needs DB lookups, blacklist was already fetched by the
// caller (it's an LW call we don't want to duplicate).
