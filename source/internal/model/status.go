...
// IsFinal reports whether the status is a terminal state — the application will
// not transition any further without a new application being created.
...
func IsActive(status string) bool {
	return status == StatusPending || status == StatusChecking || status == StatusPendingApproval
}
