...
var (
	...
	// PR #435: AZMK kredit müqavilə nömrəsi (azmk_loan_id, VARCHAR(50)) —
	// hərf/rəqəm birləşməsi (məs. "HO0030210"). Çox qısa/uzun dəyərləri kəsirik.
	azmkLoanIDRegex = regexp.MustCompile(`^[A-Za-z0-9]{3,50}$`)
)
...
// isValidAzmkLoanID checks if an AZMK loan contract number is valid
// (3-50 alphanumeric characters, e.g. "HO0030210"). PR #435: partner
// video-url endpoint-i kredit müqavilə nömrəsi ilə axtarır.
func isValidAzmkLoanID(loanID string) bool {
	return azmkLoanIDRegex.MatchString(strings.TrimSpace(loanID))
}
