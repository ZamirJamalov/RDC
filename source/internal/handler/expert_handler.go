...
func (h *ExpertHandler) Queue(w http.ResponseWriter, r *http.Request) {
	// The service layer doesn't yet have a "list pending" method — for now
	// we return a placeholder. A future PR will add ListByStatus to the repo.
	writeExpertJSON(w, http.StatusOK, map[string]interface{}{
		"status":  "pending_approval",
		"message": "Use GET /api/applications/{id}/status to check individual applications",
		"note":    "Queue listing will be implemented with a dedicated query in a follow-up PR",
	})
}
