...
// UpdateAddress handles PUT /api/applications/{id}/address.
// PR #245: ekspert faktiki ünvanı redaktə edir və DB-də saxlanılır.
func (h *ApplicationHandler) UpdateAddress(w http.ResponseWriter, r *http.Request) {
	id, err := strconv.Atoi(r.PathValue("id"))
	if err != nil || id <= 0 {
		writeError(w, http.StatusBadRequest, "invalid application id")
		return
	...
	app, err := h.service.UpdateActualAddress(r.Context(), id, req.ActualAddress)
	if err != nil {
		slog.Error("update address failed", "application_id", id, "error", err)
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
...
func (h *ApplicationHandler) BackfillRegistrationAddress(w http.ResponseWriter, r *http.Request) {
	id, err := strconv.Atoi(r.PathValue("id"))
	if err != nil || id <= 0 {
		writeError(w, http.StatusBadRequest, "invalid application id")
		return
	...
	app, err := h.service.BackfillRegistrationAddress(r.Context(), id)
	if err != nil {
		slog.Error("backfill registration address failed", "application_id", id, "error", err)
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}

	writeJSON(w, http.StatusOK, map[string]string{
		"registration_address": app.RegistrationAddress,
